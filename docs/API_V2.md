...
`npm run build` first creates a deterministic allowlisted public snapshot and then imports only that snapshot. It requires schema `002`, SQLite integrity/foreign-key success, a nonzero derived count, and equality between the snapshot and v2 projection. `/api/health` requires both v1 and v2 readiness and equal counts.
...
### `GET /api/v2/companies?limit=&cursor=&q=&region=&status=&stage=`
