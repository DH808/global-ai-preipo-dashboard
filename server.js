...
function publicCleanText(value) {
  return String(value ?? '')
    ...
    .replace(/data lakehouse\+AI平台/gi, 'Data Lakehouse 与 AI 平台')
    .replace(/last round/gi, '最后一轮估值')
    ...
    .replace(/AI infrastructure target/gi, 'AI 基础设施标的')
    ...
    .replace(/Access route:/gi, '接触路径：')
    .replace(/route:/gi, '路径：')
    .replace(/latest round:/gi, '最近一轮：')
    .replace(/open claims:/gi, '待核验 claims：')
    .replace(/strategic investors \+ semiconductor\/customer design-win checks/gi, '战略投资人 / 半导体客户渠道')
    .replace(/Ask\s+针对\s+/gi, '核验 ')
    .replace(/public-获取 review/gi, '公开资料核验中')
    .replace(/commercialization 和/gi, '商业化进度和')
    .replace(/pursue via/gi, '通过以下路径推进：')
    .replace(/only 与 price discipline/gi, '并严格遵守价格纪律')
    .replace(/production timing/gi, '量产时间')
    .replace(/margin model/gi, '利润率模型')
    .replace(/strategic co-invest/gi, '战略共同投资')
    .replace(/customer design-win proof/gi, '客户设计定点证明')
    .replace(/customer names under NDA/gi, 'NDA 下的客户名单')
    .replace(/realized 收入 by year/gi, '分年度确认收入')
    .replace(/customer concentration/gi, '客户集中度')
    .replace(/cash-flow definition/gi, '现金流定义')
    .replace(/AI networking exposure/gi, 'AI 网络业务暴露')
